...
send 多行粘贴与 label 端点融合（对应 `GOAL.md`，方案 P0007，登记日 2026-08-31）。**2026-08-31 达成：全链路验收过（含中文多行、stale pipe 自愈、daemon 亡引导）。** 下一刀候选（按依赖序）：`oma init` 接 deploy 层（hook/skill 部署接 CLI）、`oma run` 委派与任务映射、REPL + HTTP 观察面、真四路拉通（需装齐）。
