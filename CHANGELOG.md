...
- **v0.5.4 封版**：D28 根修加第 2 轮扩面加 codex review 九条修复。第 2 轮（用户同日两令）：yolo 与非阻塞键全量用户级（四家用户配置、项目旧键 init 等值退役、doctor 判据随迁，覆盖 D25 项目级口径；git 历史佐证用户级诉求自 POC 期逐面翻正）；状态栏中文目录乱码根修（stdin 字节级 UTF-8 解码，防 CP936 误解码「缁跨洘」形）。codex review：F1 codex 信任键源根修（键源 = hooks.json 路径，双版本无 bypass 活体落盘实证，P0010 旧账闭案记 M061）、F2 doctor 逐键对账、F3 家目录短路守卫、F4 外来定义保留、F5-F9 锁与辨形与回落形态加固。herdr codex review 对齐后发；三平台资产加边车同形态。
