...
| D28 | 状态栏跨项目失效根修：hook 采集面迁用户级（用户裁 2026-09-11「hook 应用户全局」）。现象 = 未 init 项目状态栏 claude:unknown；实证根因 = 状态栏命令在用户级而状态采集 hook 与 shim 落项目级（`.oma/hooks/` 写项目相对 `.oma/state/`），两 agent 注册面还不一致（codex 用户级、claude 项目级）；状态文件按 agent 单键，herdr 多 claude 会话并发互踩 | 已采纳 | 第 1 轮（2026-09-11 用户裁定随诊断一次给齐）：1) hook 注册与 shim 常驻用户级（`~/.oma/hooks/`，codex 形态对齐，四家统一）；2) 状态文件按 session 分键（`~/.oma/state/<agent>-<session>.json`）防多会话互踩；3) 状态栏按当前 session 读；4) `oma init` 落用户级后项目 `.oma/hooks` 退役（含旧注册迁移清理）；5) 修完发小版本 v0.5.4，资产 digest 到后对端验锚；6) 交付前经 herdr 让 codex 独立 review | 进行中：落码 shim/deploy/statusline/hook/doctor/verify 六面加测试门禁加文档同步；P0045 归档加 v0.5.4 发版 |
