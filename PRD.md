...
| D28 | 状态栏跨项目失效根修：hook 采集面迁用户级（用户裁 2026-09-11「hook 应用户全局」）。现象 = 未 init 项目状态栏 claude:unknown；实证根因 = 状态栏命令在用户级而状态采集 hook 与 shim 落项目级（`.oma/hooks/` 写项目相对 `.oma/state/`），两 agent 注册面还不一致（codex 用户级、claude 项目级）；状态文件按 agent 单键，herdr 多 claude 会话并发互踩。第 2 轮扩面：yolo 与非阻塞模式也全量用户级（「包括yolo模式和非阻塞模式也要是用户级别」；用户点名 git 历史佐证同类诉求反复提出：POC 期 93ac1f9「不改家目录」约束起，状态栏 P0027、kimi M058、hook D28、yolo 本轮，逐面翻正） | 已采纳 | 第 1 轮（2026-09-11 用户裁定随诊断一次给齐）：1) hook 注册与 shim 常驻用户级（`~/.oma/hooks/`，四家统一）；2) 状态按 session 分键（`~/.oma/state/<agent>-<session>.json`）防多会话互踩；3) 状态栏按当前 session 读；4) `oma init` 落用户级后项目 `.oma/hooks` 退役；5) 修完发小版本 v0.5.4，资产 digest 到后对端验锚；6) 交付前经 herdr 让 codex 独立 review。第 2 轮（2026-09-11 同日）：7) yolo 与非阻塞键全量用户级（四家用户配置，覆盖 D25 项目级口径），项目级旧键 init 退役。第 3 轮（2026-09-11 同日追裁）：8) yolo 命令分两级显式，`--yolo` 用户级（缺省全套亦用户级）、`--project-yolo` 项目级（互斥；项目级旧口径收编为显式旗标），doctor 双级接受；9) 第 4 令：双级冲突要可诊断可告警成 CTA（doctor 三家 yolo 双级并读，值不同打 warn 加对齐指引） | 进行中：六面落码加三轮裁定加 codex review 九条修复加 R2 六条残留全清全绿；P0045 归档；v0.5.4 发版 |
