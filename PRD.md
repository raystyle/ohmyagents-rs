...
| D07 | oma 收窄为配置 agent、hook、编排（用户裁定 2026-09-05「oma 以后只管配置 agent 和 hook 和编排，不管 agent 的升级和安装」）：agents install/update 加 deprecated 指向 ome，`catalog\agents.toml` 数据权威注记转 ome；登录态 / hook 形态 / 状态栏 / 会话健康四类检查归 agents 域 | 已采纳 | 第 3 轮（随 ome 仓 D07 三轮六裁，跨仓同轮） | ome 仓 D07 切片 4 配合；本仓 TODO 迁册批 |
