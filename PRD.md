...
| D08 | ome只管软件部署；oma 要对 hook、状态栏和 agent 配置的检查 | 已澄清 | 第 1 轮（2026-09-07 用户裁定，纠正「ome doctor 调用 oma」的误读）：检查面留 oma doctor，不收进 ome；触发项 Grok 状态栏显示错误 | 本轮热修 Grok 状态栏 ASCII 安全输出（M046）；完整立项待检查面缺口盘点 |
