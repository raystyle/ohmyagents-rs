...
/// 根，跨项目共享、hst 轮换无痛）。Windows 用**无引号正斜杠绝对路径**加
...
const COMMAND_MAP: &[(&str, &str)] = &[
    (
        ...
        "部署 hook/skill 与 yolo 键（幂等；hook 注册用户级常驻 ~/.hst/hooks/ shim，状态按 session 分键，项目旧注册自动退役；yolo 两级显式：--yolo 用户级全机、--project-yolo 项目级覆盖，D28）",
    ),
    (
        "hst doctor",
