...
3. **发现问题时**
   - 可以：任何问题都参照现有文档逻辑、结构与 `INDEX.md` 自修正，走五步闭环（循环自迭代）——**定位**：先搜 INDEX 与相关文档，确认是否已有规则、研究或参考覆盖；**归类**：文档错修文档、规则缺补规则（AGENTS 或对应细则）、知识缺落研究（六态）、出错模式记 mistakes、验证过的做法沉淀 references；**修正**：改在源头，下游引用、索引与三原语同步；**验证**：`rumdl check .` 加 `.tools` 三个 md 扫描（`md-ref-scan.py` 断链、`md-heading-scan.py` 标题括号、`mdcharlint.py` 四类禁用字符封闭清单门禁），涉及结构再对账 INDEX 与磁盘；**提交**：一事一提交，diary 记钩子。
...
- **设备码登录引导**：`oma agents login <grok|kimi> [--timeout N]`（S026 落地：起 `grok login --device-code` / `kimi login` 子进程、只抽 `login.url=`/`login.code=` 干净输出不转发原始 stderr——设备码流天生跨机，URL 加 code 拿到任何机器完成、等浏览器侧完成（缺省 600s、0 不限时）、以落盘凭据确认非单信标记；claude/codex 走各自原生登录）
- **密钥一钥两密文与四 shell 懒注入**：`oma agents secrets init|set <KEY>|env --shell <pwsh|bash|zsh|nu>|inject|status`（S031，对齐 ohmycloud D20 与 ohmypwsh 懒注入）：oma 自管根落 `app.key`（32B、0600 原子写）加 `identity.enc`（AES-256-GCM 包裹 SOPS 标准 age 身份，`oma:v1:` 标记）加 `secrets.yaml`（SOPS 制密文，sops 二进制加工、值 base64）；解密链全程内存 app.key → identity.enc → SOPS_AGE_KEY → vault；`inject` 向 pwsh/bash/zsh/nu 四 profile 写标志行包裹的懒注入块（幂等），交互 shell 启动现场解密只写会话 env，明文不常驻注册表；秘密不进 argv（set 走 stdin）、输出 redacted
...
- **配置状态栏**：`oma agents statusline [名]`（四家：claude settings.json 幂等合并 statusLine 块、codex config.toml [tui] status_line 整段替换、kimi tui.toml [status_line].command、grok config.toml [ui.status_line] type=command——S025 矩阵；pwsh 脚本释放 oma 数据根 statusline/，脚本内强制 UTF-8 输出防 CP936 下 emoji 变 `??`；pwsh 未检出打 `statusline.pwsh=missing` 警告）
- **hook 写状态加密钥拦截**：`oma hook`（agent lifecycle hook 调用；stdin JSON 或参数；写 `OHMYAGENTS_STATE_FILE`；缺环境则静默。不连 rmux 管道。**密钥拦截闸**（S030）：PreToolUse/UserPromptSubmit 命中 block 级密钥 → exit 2 拒工具调用（stderr 掩码原因回给模型），PostToolUse 只观察；八层防误报——精确前缀、实值比对（env + providers.toml 明文）、熵值门、stopwords、语料拼接豁免、password warn-only、日志掩码、fail-open）
...
- **部署项目级 hook/skill/yolo**：`oma init [--project PATH]` 全套（yolo 键加 hook/skill，S015 矩阵，幂等不改家目录）；`--yolo` 仅键；`--pretrust` 追加家目录信任库。**四环境自适应**（P0027，矩阵见 S024）：claude/grok 探针命中 PATH 写 bare `oma`（各 OS 用自己的 PATH 解析，粘性不降级），codex 按字段所有权各侧只写本侧（`command`/`commandWindows`），共享项目目录 Windows 与 WSL 双侧并存不互踢；输出 `init.hooks.form=` 标记形态
- **和解拉起**：`oma spawn [--agents a,b] [--stub] [--project PATH]`（P0024：会话不在新开，在则活路附加、死路重开；**精确集合与提供商别名**：`--agents` 支持 `claude@zhipu,codex@deepseek` 形态按别名注入 env/argv（providers.toml）；给几路就几路，多余路自动收掉出 `removed`，**先补后收**防空会话；布局按路数自适应：1 全屏、2/3 左右列分、4 路 tiled 2x2；命令面只见 agent 实例，窗格复杂性绑在背后；拉起后自动跑一轮信任框 settle——白名单只碰信任/升级屏，任务级确认永不自动按；**新拉路等待就绪**（idle 稳定/working/画面变化任一，20s），有阻塞或死路打 `spawn.alert=`）；阻塞用 `doctor`/`status` 诊断，不在主命令里长时间 `wait_ready` 卡住委派
...
- **起 web 镜像**：`oma web [agent] [--spectator] [--ttl N] [--no-pin]`（P0021/P0022 已落地）：缺省整会话镜像（全窗格带分屏），给 agent 单 pane；官方域中继走公网（E2EE 加密、缺省 PIN 防外发），**官方域 + `--no-pin` 组合会打 PUBLIC-RELAY-NO-PIN 显著警示**（任何拿到 URL 者可在 TTL 内接入，operator 即可打字）；`oma serve` 的 `GET /` 即 web-mirror-server 主页（本地前端、自动免 PIN、operator，打开即多路窗格）；HTTP `POST /share`（会话）/`POST /share/{agent}`（单路）/`GET /share`/`DELETE /share/{id}/stop`
