...
5. **经验沉淀时（强规则，G004）**
   - 可以：成功的 plan 沉淀归 `docs\proven\`（方案与过程）；研究被实证后的做法与多次错误后沉淀成的正确工作流进 `docseferences\` 并挂意图路由或 R002；错误经验踩坑当场记 `docs\mistakes\`（同根因聚合）；同型坑二犯以上把正确处理升格成 references 工作流并互指。
