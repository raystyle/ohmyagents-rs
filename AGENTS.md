...
9. **写文档时**
   - 可以：遵守 `docs\guide\G001-文档标准细则-命名写作规范与rumdl检查.md`（树形、标题干净、文件名即标题、rumdl）加 `docs\guide\G005-中英文技术文档字符与标点硬禁令.md`（四类禁用字符：破折号、箭头、emoji、非法全角；豁免区与替代写法；新写与改动立即生效，存量清零前 `mdcharlint.py` 不强制进门禁）。
