...
- **设备码登录引导**：`oma agents login <grok|kimi> [--timeout N]`（S026 落地：起 `grok login --device-code` / `kimi login` 子进程、stderr 转发加 `login.url=`/`login.code=` 机读标记、等浏览器侧完成（缺省 600s、0 不限时）、以落盘凭据确认非单信标记；claude/codex 走各自原生登录）
