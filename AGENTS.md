...
- **配置状态栏**：`oma agents statusline [名]`（claude settings.json 幂等合并 statusLine 块、codex config.toml [tui] status_line 整段替换；pwsh 脚本释放 oma 数据根 statusline/，脚本内强制 UTF-8 输出防 CP936 下 emoji 变 `??`；pwsh 未检出打 `statusline.pwsh=missing` 警告）
