...
- **oma 自更新**：`oma self update [--channel dev|latest] [--repo owner/name] [--git] [--force]`（缺省 **dev 滚动源**：CI 每推 main 覆盖发布的 prerelease，按资产 sha256 判新；`latest` 走正式版（v* tag 触发构建）；Windows rename 舞步自替换；无 release 体面降级 `--git` 源码安装；机制见 S028）
