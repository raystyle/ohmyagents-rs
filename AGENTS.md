...
- **起 HTTP 编排面**：`oma serve [--port N] [--project PATH]`（P0011 已落地，需 `--features server` 构建）：六操作 RESTish 加 JSON 信封加 SSE 画面；**主页即 web 镜像**（打开就是多路窗格，P0022；配置 dashboard 已删，编排走 CLI/API/MCP）；只绑 127.0.0.1；curl 全绿口径见 `docs\references\R002`
...
已落地：`check`、`init`（全套）、`doctor`、`agents`、`agents install`、`agents update`、`hook`、`spawn`、`status`（TTY 表格）、`send`、`cleanup`、`run`、`settle`、`trace` 六视图、`serve`（HTTP 编排面加网页可视化）、`mcp`（stdio 九 tools）、REPL（裸 `oma`，内嵌编排面）、`web`（web 镜像，整会话缺省）、`completions`、六会话命令 `--json` 信封。设计命令全部落地；新想法走 G003 五步再立项，禁止把未验收口径写成已可跑。
