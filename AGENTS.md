...
- **和解拉起**：`oma spawn [--agents a,b] [--stub] [--project PATH]`（P0024：会话不在新开，在则活路附加、死路重开；**精确集合**：`--agents` 给几路就几路，多余路自动收掉出 `removed`；布局按路数自适应：1 全屏、2/3 左右列分、4 路 tiled 2x2；命令面只见 agent 实例，窗格复杂性绑在背后；不阻塞；拉起后自动跑一轮信任框 settle——白名单只碰信任/升级屏，任务级确认永不自动按）；阻塞用 `doctor`/`status` 诊断，不在主命令里长时间 `wait_ready` 卡住委派
