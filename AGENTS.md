...
- **起官方 web 镜像**：`oma web [agent] [--spectator] [--ttl N]`（P0021 已落地）：rmux web-share 接管，operator 真可操作 attach；HTTP 对应 `POST /share/{agent}`、网页状态卡有按钮
...
已落地：`check`、`init`（全套）、`doctor`、`agents`、`agents install`、`agents update`、`hook`、`spawn`、`status`（TTY 表格）、`send`、`cleanup`、`run`、`settle`、`trace` 六视图、`serve`（HTTP 编排面加网页可视化）、`mcp`（stdio 九 tools）、REPL（裸 `oma`，内嵌编排面）、`web`（官方 web 镜像）、`completions`、六会话命令 `--json` 信封。设计命令全部落地；新想法走 G003 五步再立项，禁止把未验收口径写成已可跑。
