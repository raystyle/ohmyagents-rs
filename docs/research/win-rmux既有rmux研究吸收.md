...
> 2026-08-29 全量吸收，2026-08-31 压缩重写：约六成结论已由专项研究、MISTAKES 与本机 POC 实测承载，本篇只留 win-rmux **独有**的四块事实（仓库地图、进程模型与独有行为行、hardened-guard 八条、命令核验表）。通用结论请查专项：《drive铁律与三段式粘贴》《agent状态判断-通道与分层》《rmux-sdk最佳开发实践与验证poc》《yolo与无阻塞启动-配置落盘与无头分路》与 `docs\mistakes\MISTAKES.md`。
