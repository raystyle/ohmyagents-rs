...
> 角色：**选 Rust 库与项目的研究操作手册**。AGENTS 写 Rust 规则「先查最流行、最稳定的库」的细则载体，两法浓缩：crates.io 通道（`rust库搜索研发分析法-CLI与API选型实证.md`）与 GitHub 通道（`github搜索研究法-gh命令与仓库深读实证.md`），均为 2026-08-31 本机实证；为什么与完整证据链见两篇研究。
