...
grok 单路全屏会话（用户验收精确集合场景顺带委派）跑 14m54s，报告高/中在 scrollback 之外，尾部三个重点缺口全部核实为真并当日修（`5050e83`）：

- **stalled 死循环**（我二轮重构引入）：stalled 只告警不退出，外层重扫同 marker 立即重按到 deadline——防重复提交的初衷失效。修：stalled 路冷却到窗口结束（HashSet 跳过）。
- **新开与纯附加不 relayout**：spawn 的 split 序列（Right/Right/Down）让 3 路新开成上 2 下 1，不是用户要的三列。修：spawn 尾与 reconcile 尾**无条件** relayout（幂等毫秒级，「有动作才排」的省略不留）。实测 3 路 stub 新开三列（40/39/39 全高）。[实证]
- **自动 settle 只在 CLI 与 HTTP**：MCP 的 oma_spawn 漏。修：MCP 侧 gate 外补调，三通道同语义；SettleParams.wait 文档订正为「全局扫描窗口秒数」。
- grok 另提「先收后补时序空窗」「不相交可毁会话」——精确集合按用户定调保留现语义，记档知悉。

三家 agent 对同一批代码给出互补视角（codex 抓并发与协议边界、grok 抓行为语义与布局形态）——多路编排器的自举验证：**oma 编排的 agent 在给 oma 自身做 review**。

## 五轮复核：codex 经 oma task 委派

首次用 `oma task` 协议委派 review（产物落任务目录 output.md，后台收件人模式收件）——**task 协议 codex 路端到端第二次实证**。发现高 1 / 中 2 / 低 9，当日修（`683db63`）：

- **高1 blocked 被画面变化短路**（真，await_task_start 分支序）：先 idle 后弹框场景首帧 hash 变化先命中返回空，把「被阻塞」误报「已开始」——blocked 检查提前到变化信号前。
- **中2 CLI --json 丢 alerts/settle**（真）：json 分支 early-return 绕过就绪确认——补齐三通道同语义，顺带删死代码重复块。
- **中3 kill 失败孤儿 pane**（真）：先分后杀容忍 kill 失败后 manifest 已指向新格，旧格无人重试——两处打 `orphan pane` 强告警（可重试孤儿清单记档后续）。
- 低项顺手修四件：task_wait 校验 output 非空（3s 宽限）、task_new send 失败回滚目录、`task --project X list` 内外层参数兜底、task show 补打 text。低 4/9/12 为文档取舍类记档。

五轮链总结（codex→codex→grok→kimi→codex）：每轮都抓出上一轮新引入的至少一个真缺陷，且形态互补（并发/语义/时序/契约）——**多 agent 交叉复核是有效的质量手段，oma task 协议让它成为标准工作流**（委派即产物、收件人模式即闭环）。
