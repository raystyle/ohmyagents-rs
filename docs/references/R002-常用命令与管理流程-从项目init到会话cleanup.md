...
| 起官方 web 镜像 | `oma web [agent] [--spectator] [--ttl N] [--project PATH]` | rmux web-share 接管：operator 可操作真 attach（缺省）、spectator 只看；缺省 agent 全会话各起一路；打印 URL/PIN/过期（PIN 等同键盘权限勿外传） |
