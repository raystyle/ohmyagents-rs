...
| 设备码登录引导 | `oma agents login <名> [--timeout N]` | grok/kimi 专用（claude/codex 走各自原生登录）。起 `grok login --device-code` / `kimi login` 子进程：stderr 逐行转发加抽 `login.url=`/`login.code=` 机读标记（输出契约源码实证 S026），等浏览器侧完成（缺省 600s、0 不限时，超时杀进程），成功判据 = 退出 0 **且** doctor 登录态判据过（落盘凭据为准，不单信成功标记）；失败带 `login_state=` 与最后失败行 |
