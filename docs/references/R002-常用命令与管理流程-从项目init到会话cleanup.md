...
| 起 web 镜像 | `oma web [agent] [--spectator] [--ttl N] [--no-pin] [--project PATH]` | rmux web-share 接管：缺省**整会话镜像**（一个 URL 全窗格、operator 可编辑、带分屏控制）；给 agent 则单 pane；官方域前端（PIN 防外发，`--no-pin` 显式关）；serve 起的走本地前端自动免 PIN。URL/PIN/过期打 marker 行（PIN 等同键盘权限勿外传） |
