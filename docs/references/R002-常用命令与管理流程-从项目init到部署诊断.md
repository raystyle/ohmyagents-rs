...
| 意图 | 命令 | 行为细则 |
| --- | --- | --- |
| 部署项目全套 | `oma init [--project PATH]` | yolo 键加 hook / skill 部署：`.claude/settings.json`（yolo 加 hooks）、`.codex/hooks.json` 加 `config.toml`（yolo 加 features.hooks）、`.grok/hooks/ohmyagents-state.json`、四家 skill 目录、AGENTS / CLAUDE.md（仅缺失时）。幂等合并保留外条目，不改家目录。D27 shim 解耦（2026-09-10 用户裁）：init 先落自包含状态写入 shim 到 `.oma/hooks/`（三份脚本全侧落齐跨 OS 并存：`oma-state.cmd` 加 grok 包装 `oma-state-grok.cmd` CRLF、`oma-state.sh` bash 或 mac zsh shebang 加 Unix chmod 755），各家注册指向 shim，state 通道零 oma 依赖（oma 可任意时刻轮换升级）；secretguard 由 shim fail-open 委托（PreToolUse / UserPromptSubmit 时 oma 在位则转发 payload 透传退出码 2，不在位放行）。cmd shim 两级形态：部署前探 PATH 上 jq（jq 归 ome 部署，用户裁 2026-09-10），在位落 jq 解析版（字段提取与 state JSON 生成全走 jq `--arg`，ts 取 `now\|floor`），缺位落 findstr 回落版加 `init.hooks.warn` 指向 `ome install jq`；sh 侧 sed 是 POSIX 基线。注册形态按消费方 shell：claude / grok（settings.json，经 PowerShell，M047）用 `& "cmd 路径" 名`；grok Windows 只认可整串 spawn 单路径（M048）指向 baked 包装；codex 字段所有权各侧只写本侧（`command` / `commandWindows`，P0027），`command` 为 schema 必填（Windows 侧新部署无保留值时落 bare `oma hook --agent codex` 兜底，M055），commandWindows 为 PowerShell 调用操作符 `& "cmd 路径" codex`（M057 根修：codex hook 经会话环境 shell 执行、Windows 缺省 PowerShell，直引号路径是 ParserError；M056 的 cmd 直引号结论系验证通道错误已订正）；Unix 侧 `command` 为 `"sh 路径" codex`；config.toml `[hooks]` 只留 state 信任键，非 state 定义键部署时清扫（单一来源 = hooks.json，消 codex 双 representation 警告）。共享项目目录：codex 双字段两侧并存不互踢；claude / grok 单 command 字段，本侧重部署收敛本侧形态（异侧消费面待其本侧 init）。陈旧判据 = ours 且不等于本次 shim 命令（老 bare / 旧 exe / 搬迁旧 shim / 重复条目一律弃后补单条）。输出 `init.hooks.form=shim` 标记形态 |
