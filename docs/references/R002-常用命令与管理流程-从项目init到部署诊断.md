...
| 意图 | 命令 | 行为细则 |
| --- | --- | --- |
| hook 写状态加密钥拦截 | `oma hook [event]` | 双职责。其一状态落盘：读 stdin JSON（`hook_event_name` / `hookEventName`）或参数，写 `OHMYAGENTS_STATE_FILE`；缺该环境变量则回退写 `<payload cwd 的 .git 根>/.oma/state/<agent>.json`，回退要求 agent 名非空（`OHMYAGENTS_AGENT` 或 `--agent`）且 cwd 向上 8 层内有 `.git`，两条件缺一则静默 exit 0 不落盘（D40 四端验收红根因：无头直测漏 `--agent`，实现无缺陷）。状态供状态栏 `agent:state` 机读标记消费（S025）。D27 起常规路径不走本命令：项目 hook 注册指向 `.oma/hooks/` 自包含 shim（见 init 行），本命令保留为手动入口与 shim 的 secretguard 委托目标（含完整 notification 形状解析，shim 简化形态的补充面）。其二密钥拦截闸（S030）：PreToolUse / UserPromptSubmit 命中 block 级密钥则 exit 2 拒工具调用（stderr 掩码原因回给模型），PostToolUse 只观察。八层防误报：精确前缀、实值比对（D20 后只看环境变量；D23 起完整 token 边界匹配：值须两侧无 [A-Za-z0-9_.-] 类字符才算命中，杀模型别名连字符超集误报 #9，真实密钥完整值仍 block；masked 形如 `名[头4字符…长度]` 可定位不泄密）、熵值门、stopwords、语料拼接豁免、password warn-only、日志掩码、fail-open |

### 2.4 项目部署

| 意图 | 命令 | 行为细则 |
| --- | --- | --- |
| 部署项目全套 | `oma init [--project PATH]` | yolo 键加 hook / skill 部署：`.claude/settings.json`（yolo 加 hooks）、`.codex/hooks.json` 加 `config.toml`（yolo 加 features.hooks）、`.grok/hooks/ohmyagents-state.json`、四家 skill 目录、AGENTS / CLAUDE.md（仅缺失时）。幂等合并保留外条目，不改家目录。D27 shim 解耦（2026-09-10 用户裁）：init 先落自包含状态写入 shim 到 `.oma/hooks/`（三份脚本全侧落齐跨 OS 并存：`oma-state.cmd` 加 grok 包装 `oma-state-grok.cmd` CRLF、`oma-state.sh` bash 或 mac zsh shebang 加 Unix chmod 755），各家注册指向 shim，state 通道零 oma 依赖（oma 可任意时刻轮换升级）；secretguard 由 shim fail-open 委托（PreToolUse / UserPromptSubmit 时 oma 在位则转发 payload 透传退出码 2，不在位放行）。cmd shim 两级形态：部署前探 PATH 上 jq（jq 归 ome 部署，用户裁 2026-09-10），在位落 jq 解析版（字段提取与 state JSON 生成全走 jq `--arg`，ts 取 `now\|floor`），缺位落 findstr 回落版加 `init.hooks.warn` 指向 `ome install jq`；sh 侧 sed 是 POSIX 基线。载荷保真边界（验收 review 记）：cmd shim 用 more 抓 stdin 会把 TAB 换空格加尾部 CRLF（.sh 侧 cat 无此改写），对 secretguard 语义无实害（token 形态对空白改写不敏感），cmd 无字节级 tee 原语，记为已知边界。注册形态 M059 统一：Windows 一律**无引号正斜杠绝对路径**加参数（`D:/路径/.oma/hooks/oma-state.cmd 名`）：settings.json 是双消费者字段（claude 本体经 /usr/bin/bash -c 加 grok 经 PowerShell，M047/M059），`&` 调用操作符炸 bash、双引号路径炸 PS，唯此形态 bash / PS / cmd 三吃（三 shell 实测加真 codex 加真 claude 会话活体验证）；边界：项目路径含空格三 shell 全裂，init 落 warn 提示搬迁。grok Windows 只认可整串 spawn 单路径（M048）指向 baked 包装；codex 字段所有权各侧只写本侧（`command` / `commandWindows`，P0027），`command` 为 schema 必填（Windows 侧新部署无保留值时落 bare `oma hook --agent codex` 兜底，M055；codex hook 经会话环境 shell 执行、Windows 缺省 PowerShell，M057 实证）；Unix 侧 `command` 为 `"sh 路径" codex`；config.toml `[hooks]` 只留 state 信任键，非 state 定义键部署时清扫（单一来源 = hooks.json，消 codex 双 representation 警告）。共享项目目录：codex 双字段两侧并存不互踢；claude / grok 单 command 字段，本侧重部署收敛本侧形态（异侧消费面待其本侧 init）。陈旧判据 = ours 且不等于本次 shim 命令（老 bare / 旧 exe / 搬迁旧 shim / 重复条目一律弃后补单条）。输出 `init.hooks.form=shim` 标记形态 |
...
| 无头验收 agent | `oma agents verify [名...] [--timeout N]` | 两家一层四行（D17，S033 两层判据，kimi 面 2026-09-10 勘误见下）。状态栏层：mock JSON `{}` 直跑 `~/.oma/statusline/oma-statusline.ps1` 断言首行 `<agent>:` 机读标记（codex 无外部命令面，断言 `[tui] status_line` 含内置项 ID 标 builtin）。hook 层：临时 git 项目 in-process 部署 hook 后拉起无头（claude `-p` 加 `--dangerously-skip-permissions`、codex `exec` 加 `--dangerously-bypass-hook-trust`、grok `-p` 加 `--always-approve`、kimi `-p`），断言 `.oma/state/<agent>.json` 落盘加 state 四态合法；SessionStart 先于模型调用故不依赖 token 有效性（但 kimi 无头需已登录：未登录端报 hook no-state-file 属真阳性）。kimi 勘误（S033 原口径被全局条目掩蔽）：print 模式只触发**全局** `~/.kimi-code/config.toml` 的 `[[hooks]]`（项目级不触发、pretrust 也不解锁；0.38 至 0.42 同形实证），且 hook 命令必须是不带引号的裸命令行（引号形态静默不执行，M048 同型）；verify 走 KimiGlobalHooksGuard 临时全局注册（字节备份、追加钉当前 exe 的正斜杠命令、Drop 还原），SessionEnd print 不触发不作判据。grok 临时目录须先种子 trusted_folders（verify 自动种子加 Drop 摘除）。binary 不在记 skip 不计败；任一非跳过项 fail 退出 1 |
