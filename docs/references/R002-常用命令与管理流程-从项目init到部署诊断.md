...
> 显示名 HST（Hooks, Statusline, Trace）；仓库 `hst-rs`；CLI 二进制 `hst`。运行时数据目录是 `~/.hst`（D29；旧名 `.oma` 与 `.ohmyagents` 仅旧在则迁，见 D29 更名注记）。
