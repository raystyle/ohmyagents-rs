...
2. **Python 统一 uv 载体**：脚本头部带 `# /// script` 内联元数据，运行用 `uv run --script .tools\xxx.py`（对齐 ohmypwsh 模式；不建 venv、不装依赖进环境）。py 选库走 `docs\references\R008-项目工具Python库选型细则-pypi与uv.md`（关键词搜索只有网页；已知名字走 PyPI JSON API 加 pypistats）。
