...
- **日期**：2026-09-08。
- **起点**：D15 立项。用户三裁：oma 不改名、退化为纯部署配置工具、专注 Agent 全平台 token（secrets 域）、hook 与状态栏部署配置；编排面（rmux 后端与 spawn/send/serve/mcp 等）整体移除。

## 锚点

> 当前锚定的目标 + 推进时间线。

- **锚定的目标**：D15 oma 去编排收窄为纯部署配置工具（PRD D15 已采纳）。
