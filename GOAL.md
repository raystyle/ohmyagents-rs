...
- **日期**：2026-08-31。
- **起点**：用户定调 oma 编排操作三通道（CLI、HTTP API、MCP）加浏览器网页可视化编排——原「网页只观察」定位升级。编排核心已在 orch.rs 收敛成 Link 加六函数，三传输是薄适配；incurs 框架的三传输统一入口是现成架构参照（S016）。

## 锚点

> 当前锚定的目标 + 推进时间线。

- **锚定的目标**：三传输编排面（方案 P0011）：HTTP API、MCP、网页可视化。
