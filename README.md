...
**HST**，全称 Hooks, Statusline, Trace（原 Oh My Agents / oma，v0.6.0 更名，更名过渡期后版本线自 1.0.0 重开、不接 oma 老线）：Claude Code / Codex / Grok / Kimi 四家的 hook 落盘、状态栏、只读对话 trace、可用性诊断与 yolo 非阻塞配置，Windows / macOS / Linux（含 WSL）同一命令面。CLI 名 `hst`。与 Hipo 的 hst history picker 共存：本工具装用户目录（如 ~/.local/bin、~/.hst/bin），不覆盖 /usr/bin/hst。
